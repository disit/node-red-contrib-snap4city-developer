	...
	$(document).ready(function () {
		...
		test("drop", function () {
			...
			tests.expectedEvents = [{
					...
					pageX: expectedX,
					pageY: expectedY
				}, // A drop without an active drag moves the mouse onto the target before dropping
				{
					type: "mouseup",
					pageX: expectedX,
					pageY: expectedY
		...
		test("drag-n-drop", function () {
			...
			$testElement.simulate("drag-n-drop", {
				dx: dragX,
				dy: dragY
			});
		});
		...
		test("drop before another drag", function () {
			...
			$dragElement1.simulate("drag", {
				dx: drag1X,
				dy: drag1Y
			});
			$dragElement2.simulate("drag", {
				dx: drag2X,
				dy: drag2Y
			});
		});
		...
		test("interpolated drag", function () {
			...
			for (var i = 1; i <= stepCount; i += 1) {
				tests.expectedEvents.push({
					type: "mousemove",
					pageX: Math.round(dragStartX + i * dragX / (stepCount + 1)),
					pageY: Math.round(dragStartY + i * dragY / (stepCount + 1))
				});
			}
			tests.expectedEvents.push({
				type: "mousemove",
				pageX: dragStartX + dragX,
				pageY: dragStartY + dragY
			}, {
				type: "simulate-drag"
			});
		...
		test("interpolated, delayed drag using duration", function () {
			...
			$dragElement.simulate("drag", {
				...
				interpolation: {
					stepCount: stepCount,
					duration: (stepCount + 1) * stepDelay
				}
			});
		...
		test("interpolated, delayed drag-n-drop", function () {
			...
			tests.expectedEvents.push({
				...
				type: "mouseup",
				pageX: dragStartX + (stepCount + 1) * stepWidth,
				pageY: dragStartY
			}, {
				type: "simulate-drop"
			});
		...
		test("interpolated, delayed drag-n-drop on target", function () {
			...
			for (var i = 1; i <= stepCount + 1; i += 1) {
				tests.expectedEvents.push({
					type: "mousemove",
					pageX: Math.round(dragStartX + i * stepWidthX),
					pageY: Math.round(dragStartY + i * stepWidthY)
				});
			}
			tests.expectedEvents.push({
				type: "simulate-drag"
